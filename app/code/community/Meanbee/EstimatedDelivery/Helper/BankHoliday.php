...
class Meanbee_EstimatedDelivery_Helper_BankHoliday extends Mage_Core_Helper_Abstract
{
    ...
    public function getBankHolidays($region) {
        ...
        if (!in_array($region, array(Meanbee_EstimatedDelivery_Model_Source_HolidayRegions::ENGLAND_AND_WALES,
            ...
            Mage::log("Region is not allowed: '{$region}' passed.", Zend_Log::ERR, 'meanbee.log');
            return array();
        }
    ...
    public function isHoliday($date, $region) {
        ...
        if (is_string($date)) {
            if (preg_match(self::DATE_FORMAT, $date)) {
                ...
                Mage::log('Date is malformatted, must match \'' . self::DATE_FORMAT . "', '{$date}' passed.", Zend_Log::ERR, 'meanbee.log');
                return false;
            }
        } elseif (is_int($date)) {
            ...
            Mage::log('Date is maltyped, must be string, int or Zend_Date.', Zend_Log::ERR, 'meanbee.log');
            return false;
        }
    }
}
